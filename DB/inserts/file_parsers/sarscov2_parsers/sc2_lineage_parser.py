...
class SC2LineagesParser(FileParser):
    lineage_system_name = 'sc2'
    ...
    async def parse_and_insert(self):
        ...
        lineage_system_id = await find_or_insert_lineage_system(
            LineageSystem(
                lineage_system_name=SC2LineagesParser.lineage_system_name
            )
        )
        ...
        with open(self.filename, 'r') as f:
            ...
            for row in reader:
                try:
                    ...
                    lineage = get_value(row, ColNameMapping.lineage.value)
                except ValueError:
                    debug_info['skipped_malformed'] += 1
                    continue

                if re.match(r'not assigned', lineage, re.IGNORECASE):
                    debug_info['lineage_not_assigned'] += 1
                    continue
                ...
                # find lineage
                try:
                    lineage_id = cache_lineage_ids[lineage]
                except KeyError:
                    ...
                    cache_lineage_ids[lineage] = lineage_id
    ...
    def get_required_column_set(cls) -> Set[str]:
        return {cn.value for cn in {
            ...
            ColNameMapping.lineage
        }}


class ColNameMapping(Enum):
    sample_accession = 'taxon'
    lineage = 'lineage'
